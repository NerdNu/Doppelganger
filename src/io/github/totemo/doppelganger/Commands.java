...
public class Commands {
    ...
    public void onCommand(CommandSender sender, String[] args) {
        ...
        sender
        .sendMessage(ChatColor.translateAlternateColorCodes('&', "&eType /doppel help &d&osubcommand &efor detailed descriptions of subcommands."));
    }
    ...
    // ------------------------------------------------------------------------
    /**
     * Handle the /doppel info [list | shape &lt;name&gt; | creature
     * &lt;name&gt; | player &lt;name&gt;] command.
     */
     ...
     * </ul>
     * If the name is not specified, any name will match. The CommandSender can
     * be a player or a command block.
     */
    ...
    public void onCommandDoppelCoords(CommandSender sender, String[] args) {
        ...
        if (volume == null || tail.size() > 0) {
            ...
            // Command blocks output a redstone signal if result message is not
            // red.
     ...
     * use since this command is really just for command blocks, and mobs
     * spawned into a command block will be stuck.
     * 
    ...
    protected void spawnAndLog(CommandSender sender, String type, String name, Location loc) {
        ...
        if (_plugin.getCreatureFactory().isValidCreatureType(type)) {
            _plugin.spawnDoppelganger(type, name, loc).thenAccept(doppelganger -> {
                if (doppelganger == null) {
                    sender.sendMessage(String.format(
                            "%sSpawning a %s%s at (%d,%d,%d) in %s failed unexpectedly.",
                            _failureColour, type, nameClause, loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(),
                            loc.getWorld().getName()));
                    _plugin.getLogger().info(String.format(
                            "%s tried to spawn a %s named %s at (%d,%d,%d) in %s but it failed unexpectedly.",
                            sender.getName(), type, nameClause, loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(),
                            loc.getWorld().getName()));
                } else {
                    sender.sendMessage(String.format(
                            "%sSpawned a %s%s at (%d,%d,%d) in %s.",
                            _successColour, type, nameClause, loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(),
                            loc.getWorld().getName()));
                    _plugin.getLogger().info(String.format(
     ...
     * 
     * @return the Location of the specified CommandSender, or null if the
     *         sender has no Location (e.g. the console).
     */
     ...
     *         from the sender's Location, or null if not possible to determine
     *         a world. If the World was parsed from the args, the first
     *         argument is removed from the list.
     */
     ...
     * </pre>
     * 
     * If the World is not specified, the World of the CommandSender will be
     * used (if the sender has a Location).
     * 
     ...
     * @param sender the command sender. If the command sender has a Location,
     *        this may be used when required.
     * @param args the command line arguments, starting at the first keyword
     *        that introduces a volume description, possibly containing
     *        arguments after the description.
     * @return the corresponding Volume, or null on error. If a non-null Volume
     *         is returned, args will have all all volume parameters removed. If
     *         the the specified volume is invalid (null), all bets are off and
     *         args shouldn't be used again.
     */
        ...
        try {
            if (args.size() >= 3 && args.get(0).equals("sphere")) {
                ...
                if (args.get(0).equals("here")) {
                    ...
                    // If we can parse coordinates and radius then we have
                    // *enough*
     ...
     * @param doppelgangerCount the number of matching doppelgangers found in
     *        the Volume.
     */
